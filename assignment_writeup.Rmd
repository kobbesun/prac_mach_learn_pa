...
```
The result isn't good. There are plenty of prediction error in class B, D, and E. I increased the "maxcompete" and "maxsurrogate" parameters. Then I training the Rpart model again.
```{r, cache=TRUE}
...
```
The result is astonishing! I apply the model to tesing set.
```{r, cache=TRUE}
preRf <- predict(modRf, dfTesting, type = 'class')
table(preRf, dfTesting$classe)
```
The result is very good too. Therefore I decide to use the same model to predict the 20 pml-testing cases. My predictions are:
```{r, cache=TRUE}
