...
Therefore the out-of-sample error eastimate of the **Random Forests** model is 0.53%. Such error rate is very comfortable.
```{r, cache=TRUE}
modRf
```
#### 4. Test the model and predict
I apply the model to tesing set.
```{r, cache=TRUE}
preRf <- predict(modRf, dfTesting, type = 'class')
table(preRf, dfTesting$classe)
```
The result is very good too. Given the out-of-sample error esitmate and test resulte, I decide to use the same model to predict the 20 pml-testing cases. My predictions are:
```{r, cache=TRUE}
